...
public class FeatureController(IMediator mediator) : Controller
{
    ...
    {
        var model = await mediator.Send(new GetAllFeaturesQuery());
        return View(model);
    }
    ...
    {
        var model = await mediator.Send(new GetFeatureByIdQuery { Id = id });
        if (model is null)
        {
            return NotFound();
        }
    ...
    {
        await mediator.Send(new RemoveFeatureCommand { Id = id });
        return RedirectToAction(nameof(Index));
    }
}
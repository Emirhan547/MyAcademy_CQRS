...
public class PromotionController(IMediator mediator) : Controller
{
    ...
    {
        var model = await mediator.Send(new GetAllPromotionsQuery());
        return View(model);
    }
    ...
    {
        var model = await mediator.Send(new GetPromotionByIdQuery { Id = id });
        if (model is null)
        {
            return NotFound();
        }
    ...
    {
        await mediator.Send(new RemovePromotionCommand { Id = id });
        return RedirectToAction(nameof(Index));
    }
}